      ...
      if (operand.getDefiningOp()) {
        ...
        if (canFuse(def, use)) {
          ...
          if (def->getParentRegion() == use->getParentRegion()) {
            auto groupOp = rewriter.create<GroupOp>(
                use->getLoc(), use->getResultTypes(), fusionType);
...
class TcpFuseElementwiseOpsPass
  ...
  void runOnOperation() override {
    ...
    patterns.add<GenericBottomUpFuser>(context, canFuse, "elementwise");
